...
const NavbarLoaderNew2 = ({ onLoadingComplete }) => {
  ...
  useEffect(() => {
    ...
    // Los dos cuadrados viven fuera del grid (para poder viajar libremente a
    // la esquina más tarde), así que los anclamos a la posición real de su
    // celda desde el primer frame — deben verse ya formados, en su sitio,
    // antes incluso de que empiece la animación de los títulos.
    const anchorTo = (anchorEl, squareEl) => {
      if (!anchorEl || !squareEl) return;
    ...
    anchorTo(square2AnchorRef.current, square2Ref.current);
    ...
    // 3) Entrada de "Portfolio 2026", en la segunda línea
    tl.to(
    ...
    // 6) Solo quedan los dos cuadrados: paran de girar y viajan juntos a la esquina
    tl.call(() => {
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-black text-white' : 'bg-white text-black'} transition-colors duration-300`}>
      ...
      {showLoader && (
        ...
        >
          {/*
            ...
            (ver anchorTo en el efecto) para poder viajar libres después.
          */}
      ...
      {/* Segundo cuadrado: visible desde el inicio, viaja junto al principal y se desvanece al llegar */}
      <div
        ref={square2Ref}
...
export default NavbarLoaderNew2;
