...
// Efecto "roll" tipo johnnycarretes.com: cada carácter tiene 3 filas apiladas
// dentro de una máscara de una línea (overflow hidden) — vacío / fantasma /
// real — y se anima verticalmente. Como las filas 2 y 3 son el mismo texto,
// visualmente parece que "los caracteres cambian a esos mismos caracteres"
// mientras el conjunto aparece desde la nada (arranca en la fila vacía).
const ROLL = {
  inDuration: 0.6,
  inEase: 'expo.inOut',
  inStagger: 0.025,
...
// Renderiza un texto como spans "roll" y va rellenando charsRef.current con
// los nodos a animar (uno por carácter, incluyendo espacios).
const renderRollText = (text, charsRef) => {
  charsRef.current = [];
...
const NavbarLoaderNew4 = ({ onLoadingComplete }) => {
  ...
  useEffect(() => {
    ...
    // 2) "Kiko Climent" entra rodando (efecto johnnycarretes.com)
    tl.to(
    ...
    // 3) "Creative Frontend Developer" entra rodando
    tl.to(
    ...
    // 5) "Portfolio 2026" entra rodando, justo cuando llega el segundo cuadrado
    tl.to(
          ...
          */}
          <div className="grid grid-cols-[auto_auto_auto] items-center gap-x-1 gap-y-0 text-[clamp(1.0625rem,1.75vw,1.3125rem)] leading-[1]">
            <h1 className="flex whitespace-nowrap" aria-label="Kiko Climent">
              {renderRollText('Kiko Climent', titleCharsRef)}
            </h1>
            ...
            <h2 className="flex whitespace-nowrap" aria-label="Creative Frontend Developer">
              {renderRollText('Creative Frontend Developer', subtitleCharsRef)}
            </h2>
            ...
            <div className="flex items-center gap-1">
              ...
              <h3 className="flex whitespace-nowrap" aria-label="Portfolio 2026">
                {renderRollText('Portfolio 2026', subtitle2CharsRef)}
              </h3>
            </div>
          </div>
        </div>
...
export default NavbarLoaderNew4;
