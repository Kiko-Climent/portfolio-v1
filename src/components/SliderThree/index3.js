...
export default function SliderThree3({ images, project, navbarHeight }) {
    ...
    useEffect(() => {
        ...
        const initThree = () => {
            ...
            // Update curve distortion: pliegue tipo hoja de papel.
            // Las esquinas (X e Y en su máximo a la vez) se curvan en un sentido
            // mientras el centro cede en el sentido contrario; el signo se invierte
            // según la dirección del scroll (foldDirection).
            const updateCurve = (mesh, _worldPositionY, distortionFactor, foldDirection) => {
                ...
                const foldAmount = settings.bendStrength * distortionIntensity;

                for (let i = 0; i < positionAttribute.count; i++) {
                    const x = originalVertices[i * 3];
                    const y = originalVertices[i * 3 + 1];

                    const normalizedX = THREE.MathUtils.clamp(x / slideHalfWidth, -1, 1);
                    const normalizedY = THREE.MathUtils.clamp(y / slideHalfHeight, -1, 1);

                    // Inclinación tipo bisagra en el sentido del scroll
                    const tiltZ = -normalizedY * tiltAmount;

                    // Peso 0 en el centro -> 1 en las esquinas (X e Y al máximo a la vez)
                    const cornerAmount = (normalizedX * normalizedX + normalizedY * normalizedY) * 0.5;
                    // -1 en el centro -> 1 en las esquinas, con caída orgánica (no lineal)
                    const foldShape = smoothstep(cornerAmount) * 2 - 1;
                    const foldZ = foldShape * foldAmount * foldDirection;

                    positionAttribute.setZ(i, tiltZ + foldZ);
                }
            ...
            const animate = (time) => {
                ...
                slides.forEach((slide, i) => {
                    ...
                    if (Math.abs(slide.userData.currentY) < wrapThreshold * 1.5) {
                        ...
                        updateCurve(slide, slide.position.y, currentDistortionFactor, currentFoldDirection);
                    }
                });
